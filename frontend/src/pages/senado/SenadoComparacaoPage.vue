...
import { type Senador, type SenadorDetail } from '@/stores/senado'
...
const todosSenadores = ref<Senador[]>([])

onMounted(async () => {
  try {
    const response = await fetch(`${apiUrl}/api/senado/0/lista`)
    if (!response.ok) throw new Error('Falha ao buscar todos os senadores')
    const data = await response.json()
    todosSenadores.value = data.senadores.map((s: any) => ({
      id: s.codigo,
    ...
    console.error('Erro ao carregar senadores:', e)
  }
})
...
const compareSenadores = async () => {
  ...
  try {
    const response = await fetch(`${apiUrl}/api/senado/0/comparar?id1=${selectedA.value.id}&id2=${selectedB.value.id}`)
