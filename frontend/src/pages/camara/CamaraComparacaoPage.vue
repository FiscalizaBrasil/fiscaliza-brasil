...
import { type Deputado, type DeputadoDetail, type Despesa } from '@/stores/camara'
...
const todosDeputados = ref<Deputado[]>([])

onMounted(async () => {
  try {
    const response = await fetch(`${apiUrl}/api/camara/0/lista`)
    if (!response.ok) throw new Error('Falha ao buscar todos os deputados')
    const data = await response.json()
    todosDeputados.value = data.map((d: any) => ({
      id: d.id,
      nome: d.nome_civil,
...
const compareDeputados = async () => {
  ...
  try {
    const response = await fetch(`${apiUrl}/api/camara/0/comparar?id1=${selectedA.value.id}&id2=${selectedB.value.id}`)
